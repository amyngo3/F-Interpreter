...
let rec eval c (e:Environment) =
    ...
    | BinaryOp (op, left, right) ->
        try
            ...
            | MINUS -> IntValue (l - r)
            | TIMES -> IntValue (l * r)
            | DIV when r = 0 -> IntValue r  // return 0 instead
            | DIV -> IntValue (l / r)
...
// ================ p sample problems ================
let pLexicalScope = LetExpression(
    Name("x"),
    IntConstant(10),
    BinaryOp(
        PLUS,
...
// p sample problem in pdf - Expected 237
let p5 = LetExpression(
    Name("bot"),
    IntConstant(3),
    LetExpression(
        Name("bot"),
...
let main argv =
    let env = Environment([])
    printfn "pLexicalScope Value: %A" (eval pLexicalScope env)
